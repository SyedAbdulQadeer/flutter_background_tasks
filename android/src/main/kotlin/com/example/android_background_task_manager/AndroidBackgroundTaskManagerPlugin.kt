...
class AndroidBackgroundTaskManagerPlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "getTaskResults" -> getTaskResults(result)
            "getTaskResult" -> getTaskResult(call, result)
            "clearTaskResults" -> clearTaskResults(result)
    ...
    private fun scheduleTask(call: MethodCall, result: Result) {
        try {
            ...
            val frequency = (arguments["frequency"] as? Number)?.toLong()
            val initialDelay = (arguments["initialDelay"] as Number).toLong()
            ...
            val inputData = Data.Builder()
                ...
                .putString("taskType", if (periodic) "periodic" else "oneTime")
     ...
     */
    private fun getTaskResults(result: Result) {
        try {
            val sharedPrefs = context.getSharedPreferences("background_task_results", Context.MODE_PRIVATE)
            val completedTasks = sharedPrefs.getStringSet("completed_tasks", setOf()) ?: setOf()
            
            val results = mutableMapOf<String, String>()
            for (taskId in completedTasks) {
                val taskResult = sharedPrefs.getString("task_$taskId", null)
                if (taskResult != null) {
            ...
            result.error("GET_RESULTS_ERROR", "Failed to get task results: ${e.message}", null)
        }
    }
     ...
     */
    private fun getTaskResult(call: MethodCall, result: Result) {
        try {
            val arguments = call.arguments as Map<String, Any>
            val taskId = arguments["taskId"] as String
            ...
            result.error("GET_RESULT_ERROR", "Failed to get task result: ${e.message}", null)
        }
    }

    /**
     * Clears all task results from storage.
     */
    private fun clearTaskResults(result: Result) {
        try {
            val sharedPrefs = context.getSharedPreferences("background_task_results", Context.MODE_PRIVATE)
            val editor = sharedPrefs.edit()
            
            // Get all completed tasks and clear their results
            val completedTasks = sharedPrefs.getStringSet("completed_tasks", setOf()) ?: setOf()
            for (taskId in completedTasks) {
            ...
            editor.remove("completed_tasks")
            editor.apply()
            
            result.success("Task results cleared")
        } catch (e: Exception) {
            result.error("CLEAR_RESULTS_ERROR", "Failed to clear task results: ${e.message}", null)
        }
    }
    ...
    companion object {
        // Old registerWith method removed as it's deprecated.
        // Modern plugins use FlutterPlugin interface instead.
    }
}
